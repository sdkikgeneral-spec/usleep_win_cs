...
```
1. QPC パス（USLP_WINDOWS || USLP_GENERATOR かつ _isWin == true かつ _qpcFreq > 0）
...
2. Stopwatch パス（Stopwatch.IsHighResolution == true）
   ...
   ※ _tickToUs = 1_000_000.0 / Stopwatch.Frequency（静的初期化時にキャッシュ）

3. TickCount フォールバック（Stopwatch.IsHighResolution == false）
   (ulong)(uint)Environment.TickCount * 1000UL
   ※ ミリ秒 → マイクロ秒変換。精度は 1ms 単位
```

### 精度

- QPC：通常 ±1 µs 以下。周波数は起動時に `_qpcFreq` へキャッシュ
...
| `_qpcFreq` | `long` | QPC 周波数（起動時にキャッシュ） |
...
| `SuppressGCTransition` | 有効（QPC 系） | 無効 | — |
